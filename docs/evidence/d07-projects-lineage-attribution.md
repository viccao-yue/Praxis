...
任务：Praxis-plugin-projects 0.1 线增量（P1-11 范围内）——①项目任务会话标题右侧显示「项目 / 项目名」chip，点击打开并聚焦项目面板；②项目任务对话中模型按官方 present 语义交付的文件自动登记资料库（source=task）并幂等关联为项目资产。
...
| 复用选择 | 直接复用官方 Slot 注册、官方交付事件、`ctx.fs` 路径解析、官方 client model（`ctx.slots.inject` + `register`）与既有业务服务注入（PraxisIdentity/PraxisProjects/PraxisLibrary）。不新建 Agent loop、事件总线、存储或传输。RPC 仅新增一个只读端点 `task-context`（项目内部 Host-Client 协议）；contracts 同批补充只读 `ProjectService.taskContext` 契约（见「实现范围」）。 |
...
- `packages/plugins/projects/src/index.ts`：新增 `Praxis-projects-attribution` 子插件（静态 inject）。
...
| 命令 | 结果 |
| --- | --- |
| `corepack pnpm --filter Praxis-plugin-projects typecheck` | 退出 0 |
| `corepack pnpm --filter Praxis-plugin-projects build` | 通过（含 ui 预构建） |
| `corepack pnpm --filter Praxis-plugin-projects test` | 9/9（7 项既有 + 归因纯函数用例组：非项目任务跳过、成功路径字段、冲突后缀重试、单文件失败隔离、事件内去重、operationId 幂等） |
