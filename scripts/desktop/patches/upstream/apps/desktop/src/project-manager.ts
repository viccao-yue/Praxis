...
// WORKDSH TEST PATCH: Praxis profile layers seeded after the built-in desktop bundles.
const WORKDSH_PROFILE_BUNDLES = [
  'workdsh-bundle',
  ...
  // Third-party MIT appearance (pinned). Pack the exact npm tarball into packed/workdsh.
  'dsh-ui-appearance',
] as const
...
export function createSeedMetadata(seedDir: string, release: DesktopRelease): void {
  ...
  const manifest: DesktopProjectManifest = {
    ...
    dsh: { profile: { bundles: [...DESKTOP_PROFILE_BUNDLES, ...WORKDSH_PROFILE_BUNDLES] } },
  }
